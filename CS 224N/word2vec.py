...
def naiveSoftmaxLossAndGradient(
    ...
    sum_denom=sum(np.exp(np.matmul(outsideVectors,centerWordVec)))
    loss=-np.log(np.exp(np.dot(u0,centerWordVec))/sum_denom)
    weights=np.array([np.dot(outsideVectors[i],centerWordVec) for i in range(n)])
    gradCenterVec=u0-np.dot(weights,outsideVectors)
    gradOutsideVecs=np.array(
        [centerWordVec-np.dot(weights,centerWordVec) for _ in range(n)])
...
def negSamplingLossAndGradient(
    ...
    ### YOUR CODE HERE (~6-8 Lines)
    u0=outsideVectors[outsideWordIdx]
    denom=[np.dot(outsideVectors[i],centerWordVec) for i in negSampleWordIndices]
    loss=np.log(sigmoid(np.dot(u0,centerWordVec)))+sum(np.log(sigmoid(denom)))
    weights=np.array([np.dot(outsideVectors[i],centerWordVec) for i in negSampleWordIndices])
    gradCenterVec=u0-np.dot(weights,outsideVectors)
    gradOutsideVecs=np.array(
        [centerWordVec-np.dot(weights,centerWordVec) for _ in range(n)])
    ### END YOUR CODE
