...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn=nn.Conv1d(embed_size,embed_size,2,padding='<pad>')
        ...
        self.h_projection=nn.Linear(hidden_size,hidden_size*2,bias=False)
        ###     self.h_projection (Linear Layer with no bias), called W_{h} in the PDF.
        self.c_projection=nn.Linear(hidden_size,hidden_size*2,bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ###         src_len = maximum source sentence length, b = batch size, e = embedding size. Note that there is no initial hidden state or cell for the encoder.
        ###     2. Apply the post_embed_cnn layer. Before feeding X into the CNN, first use torch.permute to change the shape of X to (b, e, src_len). 
        X_perm=torch.permute(X, (1, 2, 0))
        X_perm=self.post_embed_cnn(X_perm)
        ###     After getting the output from the CNN, still stored in the X variable,
        ###     remember to use torch.permute again to revert X back to its original shape.
        X=torch.permute(X_perm, (2, 0, 1))
        ###     3. - Before you can apply the encoder, you need to apply the `pack_padded_sequence` function to X.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj=self.c_projection(enc_hiddens)
